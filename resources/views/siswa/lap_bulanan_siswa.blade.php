<x-app-layout>
        ...
        <div class="flex justify-between items-center">
            <div class="flex justify-center items-center mr-2">
                ...
                @if (Auth::user()->level != 'siswa')
                    ...
                    <div class="ml-5">
                        <button type="button" x-data @click="window.dispatchEvent(new CustomEvent('open-modal', { detail: 'filter_export_absen' }))">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 md:size-8 text-slate-400 hover:text-slate-600">
        ...
        <table class="table-auto w-full border mt-3 min-w-max">
            <thead class="bg-gray-100 sticky top-0">
                <tr>
                    ...
                    <th colspan="4" class="px-3 py-2 font-medium border text-center">
                        Jumlah
                    </th>
                </tr>
                <tr>
                    ...
                    @endfor
                    <th class="px-2 py-1 border text-center bg-green-200 text-green-800">H</th>
                    <th class="px-2 py-1 border text-center bg-red-200 text-red-800">A</th>
                    <th class="px-2 py-1 border text-center bg-yellow-200 text-yellow-800">S</th>
                    <th class="px-2 py-1 border text-center bg-blue-200 text-blue-800">I</th>
                </tr>
            </thead>
            <tbody>
                @php $no = 1; @endphp
    ...
    <x-modal name="filter_export_absen" :show="$show1" max-width="lg">
                ...
                <div class="mb-4">
                    ...
                    <select name="guru" class="form-select block w-full rounded border-gray-300 shadow-sm" >
                        @if (Auth::user()->level == 'guru')
                            <option value="{{$data_guru->nama_pegawai}}">{{$data_guru->nama_pegawai}}</option>
                        @else
                            @foreach ($data_guru as $pengajar)
                                <option value="{{$pengajar->nama_pegawai}}">{{$pengajar->nama_pegawai}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
